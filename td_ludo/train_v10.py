...
# Set V10 run name BEFORE any config imports
os.environ["TD_LUDO_RUN_NAME"] = "ac_v10"

from td_ludo.models.v10 import AlphaLudoV10
from td_ludo.training.trainer_v10 import ActorCriticTrainerV10
from td_ludo.game.players.v10 import VectorACGamePlayer
...
def main():
    ...
    # Initialize model — AlphaLudoV10 "slim" (6 blocks × 96 channels, 3 heads)
    model_factory = lambda: AlphaLudoV10(num_res_blocks=args.num_res_blocks,
                                         num_channels=args.num_channels,
                                         in_channels=28)
    model = model_factory()
    print(f"[Train] Architecture: AlphaLudoV10 ({args.num_channels}ch, "
          f"{args.num_res_blocks}res, 28in, {model.count_parameters():,} params)")
    model.to(device)
