...
class TurnHistory:
    ...
    def __init__(self, K=16, embed_dim=V62_EMBED_DIM):
        self.K = K
...
class FastActor:
    ...
    def __init__(self, model, batch_size, context_length=16, config=None):
        ...
        for i in range(batch_size):
            for p in range(4):
                self.turn_histories[(i, p)] = TurnHistory(context_length, V62_EMBED_DIM)
        ...
        if decision_groups:
            ...
            for (controller, controller_id), indices in decision_groups.items():
                ...
                for idx in indices:
                    ...
                    grid = ludo_cpp.encode_state_v6(game)  # (14, 15, 15)
    ...
    def _load_ghost(self, ghost_path):
        ...
        model = AlphaLudoV62(context_length=self.context_length, num_res_blocks=10, in_channels=24)
...
def actor_worker(actor_id, batch_size, context_length,
    ...
    Model inference on CPU, game simulation on CPU.
    """
    import torch
...
def actor_worker_gpu(actor_id, batch_size, context_length,
    ...
    for i in range(batch_size):
        for p in range(4):
            turn_histories[(i, p)] = TurnHistory(K, V62_EMBED_DIM)
    ...
    while not stop_event.is_set():
        ...
        for i in range(batch_size):
            ...
            if controller in ('Model', 'SelfPlay'):
                grid = ludo_cpp.encode_state_v6(game)
        ...
        if model_decisions:
            ...
            batch_cached = np.zeros((n, K - 1, V62_EMBED_DIM), dtype=np.float32)
