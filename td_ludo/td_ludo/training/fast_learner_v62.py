...
class FastLearner:
    ...
    def __init__(self, model, device, config):
        ...
        # Running return normalization (mirrors src/trainer.py lines 80-87).
        # Critical: without this, the value head is trained against raw dense
        # returns (+1..+7), which causes the positive-bias drift documented in
        # training_journal.md Experiment 11 (V6 decay from 77% -> 57%).
        # V6.1 trained with normalized returns via trainer.py; when its value
        # head is transferred to V6.2, we MUST normalize here too or the vl
        # blows up to ~2 from update 1 and corrupts the CNN trunk.
    ...
    def _convert_coreml(self):
        ...
        example = torch.randn(bs, V62_IN_CHANNELS, 15, 15)
        ...
        ml_model = ct.convert(
            traced,
            inputs=[ct.TensorType(name='grid', shape=(bs, V62_IN_CHANNELS, 15, 15))],
    ...
    def _compute_cnn_ane(self, flat_grids_np):
        ...
        for i in range(0, total, bs):
            ...
            if actual_bs < bs:
                # Pad to batch size
                padded = np.zeros((bs, V62_IN_CHANNELS, 15, 15), dtype=np.float32)
    ...
    def add_game_data(self, game_data):
        ...
        for j in range(T):
            ...
            ctx_grids = np.zeros((K, V62_IN_CHANNELS, 15, 15), dtype=np.float32)
    ...
    def _ppo_update(self):
        ...
        # See training_journal.md Experiment 11. Raw dense returns are always
        # positive (+1..+7), which drives the value head into a positive bias
        # and causes long-term policy degradation. Normalize to zero-mean,
        # unit-std using running stats (EMA with alpha=0.01) so the value head
        # sees stable, bounded targets. On first batch after a fresh init the
        # running stats are seeded from batch stats; when resuming from a
        # checkpoint that already has stats (e.g. V6.1 transfer at
        # mean=3.244, std=2.169), those values are preserved across restarts.
        with torch.no_grad():
    ...
    def save_checkpoint(self, path, is_best=False, best_path=None):
        ...
        save_dict = {
            ...
            'return_stats_initialized': self._return_stats_initialized,
        }
    ...
    def load_checkpoint(self, path):
        ...
        try:
            ...
            if isinstance(checkpoint, dict) and 'model_state_dict' in checkpoint:
                ...
                # Restore running return normalization stats. Critical after
                # V6.1->V6.2 transfer: V6.1's value head was calibrated to
                # normalized returns (mean~3.244, std~2.169), so we must
                # preserve those stats to avoid a value-loss blow-up that
                # corrupts the trunk.
                rm = checkpoint.get('return_running_mean', None)
                rs = checkpoint.get('return_running_std', None)
                if rm is not None and rs is not None:
                    self._return_running_mean = float(rm)
                    self._return_running_std = float(rs)
                    self._return_stats_initialized = bool(
                        checkpoint.get('return_stats_initialized', True)
                    )
                    print(f"[Learner] Restored return stats: mean={self._return_running_mean:.4f} "
                          f"std={self._return_running_std:.4f}")
...
def learner_worker(trajectory_queue, stats_queue,
    ...
    model = AlphaLudoV62(context_length=context_length, num_res_blocks=10, in_channels=24)
    ...
    while not stop_event.is_set():
        ...
        if games_since_eval >= eval_interval:
            ...
            try:
                from evaluate_v6_2 import evaluate_v62_model
                eval_results = evaluate_v62_model(
