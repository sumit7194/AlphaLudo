"""
TD-Ludo V9 Game Player — Slim CNN + Temporal Transformer

Key differences from V8:
1. Uses ludo_cpp.encode_state_v9() → (14, 15, 15) instead of encode_state() → (17, 15, 15)
2. TurnHistoryV9 stores 14-channel grids, embed_dim=80
3. No freeze/unfreeze logic — CNN is always trainable (small enough for Mac)
4. Same game env, bots, reward shaping, ghost system, and composition logic as V8
"""
...
class TurnHistoryV9:
    ...
    Stores the last K turns of (grid_14ch, prev_action, cached_cnn_feature).
    """

    def __init__(self, context_length=16, embed_dim=V9_EMBED_DIM):
    ...
    def get_sequence(self):
        """
        Returns:
            grids: (K, 14, 15, 15) float32
            prev_actions: (K,) int64
            seq_mask: (K,) bool — True for padded positions
        """
        n_valid = len(self._grids)
        n_pad = self.K - n_valid

        grids = np.zeros((self.K, V9_IN_CHANNELS, 15, 15), dtype=np.float32)
    ...
    def get_cached_sequence(self):
        """
        Returns:
            cached_cnn: (K, embed_dim) float32
            prev_actions: (K,) int64
            seq_mask: (K,) bool
        """
        n_valid = len(self._grids)
...
class VectorV9GamePlayer:
    """
    V9 game player with slim CNN + temporal transformer context window.
    Uses encode_state_v9 (14 channels) and embed_dim=80.
    """
    ...
    def __init__(self, trainer, batch_size, device, context_length=16,
        ...
        for i in range(batch_size):
            for p in range(4):
                self.turn_histories[(i, p)] = TurnHistoryV9(context_length, V9_EMBED_DIM)
    ...
    def play_step(self, train=True):
        ...
        if decision_groups:
            ...
            for (controller, controller_id), indices in decision_groups.items():
                ...
                # V9: CNN is always trainable, but during gameplay we use no_grad
                # Cache CNN features for new turns
                with torch.no_grad():
                    new_grids_t = torch.from_numpy(
                        np.stack(batch_new_grids)
                    ).to(self.device, dtype=torch.float32)
                    new_cnn_features = model.compute_single_cnn_features(new_grids_t)
                ...
                for j, idx in enumerate(indices):
                    ...
                    self.trajectories[idx][cp].append({
                        'grids': batch_sequences[j][0].copy(),
                        'prev_actions': batch_sequences[j][1].copy(),
                        'seq_mask': batch_sequences[j][2].copy(),
                        'action': action,
    ...
    def _random_composition(self):
        """Generate random game composition (same logic as V8)."""
