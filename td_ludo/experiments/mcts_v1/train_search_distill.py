...
def main():
    ...
    # Student model
    print(f"[Train] Initializing fresh student (MinimalCNN14, "
          f"{args.num_res_blocks}×{args.num_channels}, {V17_CHANNELS}ch)...")
    student = MinimalCNN14(
        num_res_blocks=args.num_res_blocks,
        num_channels=args.num_channels,
        in_channels=V17_CHANNELS,
    )
    student.to(device).train()
    print(f"[Train] Student params: {student.count_parameters():,}")
    ...
    for epoch in range(args.epochs):
        ...
        for start in range(0, N - args.batch_size + 1, args.batch_size):
            ...
            policy, win_prob, _moves = student(x, mask)
