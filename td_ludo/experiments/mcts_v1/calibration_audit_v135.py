...
# V13.5 architecture: V135ProductionAdapter wrapping V135Symmetric
# (10×128, 21ch packed V18-production input, token-id-indexed output).
...
def load_v135(path, device, num_res_blocks, num_channels):
    """Load V13.5 checkpoint into V135ProductionAdapter (21ch packed input)."""
    print(f"[Audit] Loading V13.5 from {path}...")
...
def collect_calibration_data(model, device, num_games, batch_size, temperature, seed):
    ...
    while games_done < num_games:
        ...
        for i in range(batch_size):
            ...
            encoded_batch.append(encode_state_v18_production(game))
...
def main():
    ...
    model = load_v135(args.model, device, args.num_res_blocks, args.num_channels)
