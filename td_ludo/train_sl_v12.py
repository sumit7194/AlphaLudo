...
def main():
    ...
    parser.add_argument('--output', default='checkpoints/ac_v12/model_sl.pt')
    parser.add_argument('--epochs', type=int, default=5,
                        help='V10 iter 8 used 3 epochs; V11 starts with 5 since '
                             'token attention is small but new, and V11 SL is '
    ...
    # V12 doesn't take attn_dim — token attention is always at num_channels.
    model = AlphaLudoV12(
    ...
    print(f"[V12 SL]   {args.num_attn_layers} Attn layers × {args.num_heads} heads "
    ...
    if args.init_from_v10:
        ...
        if not os.path.exists(v10_path):
            print(f"[V12 SL] WARNING: --init-from-v10 set but {v10_path} not found")
        else:
            ckpt = torch.load(v10_path, map_location='cpu', weights_only=False)
            sd = ckpt.get('model_state_dict', ckpt)
            report = model.load_v10_backbone(sd)
            print(f"[V12 SL] V10 warm-start: copied {len(report['copied'])} tensors, "
    ...
    print(f"[V12 SL] LR schedule: warmup {warmup_steps} steps "
    ...
    print(f"\n[V12 SL] Starting epoch {start_epoch+1}/{args.epochs} (n_train={n_train:,})")
