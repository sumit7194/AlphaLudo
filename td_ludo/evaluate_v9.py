...
from src.game_player_v9 import TurnHistoryV9
from src.heuristic_bot import get_bot, BOT_REGISTRY
from src.config import MAX_MOVES_PER_GAME


def evaluate_v9_model(model, device, num_games=200, verbose=False,
                      bot_types=None, context_length=16):
    """
    Evaluate V9 model against heuristic bots.
    Uses greedy policy (argmax) with a rolling 14ch grid context window.
    """
    model.eval()
    ...
    for game_idx in range(num_games):
        ...
        history = TurnHistoryV9(context_length)
        last_action = 4
        ...
        while not state.is_terminal and move_count < MAX_MOVES_PER_GAME:
            ...
            if current_player == model_player:
                if len(legal_moves) == 1:
                    ...
                    grid = ludo_cpp.encode_state_v9(state)
...
if __name__ == '__main__':
    ...
    results = evaluate_v9_model(model, device, num_games=args.games, verbose=True)
