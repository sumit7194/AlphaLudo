...
Differences from train_v10.py:
  - Run name = "ac_v11" (isolated checkpoint dir)
  - AlphaLudoV11 model class with 4 ResBlocks + 2 attention layers
...
def signal_handler(sig, frame):
    ...
    if STOP_REQUESTED:
        SECOND_CTRL_C = True
        print("\n[V11 Train] Force exit requested. Saving immediately...")
    else:
        STOP_REQUESTED = True
        print("\n[V11 Train] Graceful shutdown requested. Finishing current step...")
    ...
    if os.path.exists(LOCK_FILE):
        try:
            ...
            if _is_process_alive(old_pid):
                print(f"[V11 Train] ERROR: Another instance (PID {old_pid}) is already running.")
                return False
            print(f"[V11 Train] Stale lock found (PID {old_pid} not running). Removing.")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='V11 RL Training (Exp 20)')
    ...
    if args.fresh and os.path.exists(CHECKPOINT_DIR):
        import shutil
        print(f"[V11 Train] Fresh start. Purging {CHECKPOINT_DIR}...")
        for f in os.listdir(CHECKPOINT_DIR):
            ...
            try:
                ...
                print(f"[V11 Train] Warning: couldn't delete {fpath}: {e}")
    ...
    print(f"[V11 Train] Device: {device}")
    print(f"[V11 Train] Mode:   {MODE}")

    # Build model — V11 architecture
    model_factory = lambda: AlphaLudoV11(
    ...
    print(f"[V11 Train]   {args.num_res_blocks} ResBlocks × {args.num_channels}ch + "
          f"{args.num_attn_layers} Attn layers × {args.num_heads} heads")
    print(f"[V11 Train]   dropout={args.dropout} (RL must be 0.0)")
    model.to(device)
    ...
    if args.resume:
        ...
        if loaded:
            ...
            if os.path.exists(sl_path):
                ...
                print(f"[V11 Train] Initialized from SL: {sl_path}")
            else:
                print("[V11 Train] No SL baseline found. Random weights.")
    else:
        sl_path = os.path.join(CHECKPOINT_DIR, 'model_sl.pt')
        if os.path.exists(sl_path):
            trainer.load_checkpoint(sl_path)
    ...
    for g in trainer.optimizer.param_groups:
        if g['lr'] <= 0 or args.reset_lr:
            ...
            print(f"[V11 Train] Reset LR {old_lr:.1e} → {LEARNING_RATE:.1e} ({reason})")
    ...
    _game_db = game_db
    print(f"[V11 Train] Game DB: {game_db.get_total_games()} games recorded")
    ...
    if lr_anneal_active:
        ...
        print(f"[V11 Train] LR annealing: {lr_start:.1e} → {lr_end:.1e} "
    ...
    if rl_warmup_games > 0:
        print(f"[V11 Train] LR warmup: 0 → {base_lr:.1e} "
              f"linearly over first {rl_warmup_games:,} games")

    print(f"\n{'=' * 60}")
    print(f"  V11 RL Training — Experiment 20 (ResTNet)")
    ...
    try:
        while not STOP_REQUESTED:
            ...
            if args.games > 0 and session_games >= args.games:
                print(f"[V11 Train] Reached game limit ({args.games})")
                break

            if args.hours > 0:
                if (time.time() - start_time) / 3600 >= args.hours:
                    print(f"[V11 Train] Reached time limit ({args.hours}h)")
                    break
            ...
            if current_bucket > last_eval_bucket:
                ...
                if trainer.is_stagnated:
                    print(f"\n[V11 Train] ⚠️ WARNING: No improvement for "
                          f"{EARLY_STOP_PATIENCE} consecutive evals.")

    except KeyboardInterrupt:
        print("\n[V11 Train] Keyboard interrupt")
    except Exception as e:
        print(f"\n[V11 Train] Error: {e}")
        import traceback
        traceback.print_exc()
    ...
    print(f"\n{'=' * 60}")
    print(f"  V11 RL Training Complete")
