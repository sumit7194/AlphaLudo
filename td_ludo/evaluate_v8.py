"""
TD-Ludo V8 Evaluator — V6 CNN + Temporal Transformer evaluation.

Same evaluation protocol as V6/V7 (greedy policy against bots), but
maintains a rolling context window of grid states for the transformer.
"""
...
from src.game_player_v8 import TurnHistoryV8
from src.heuristic_bot import get_bot, BOT_REGISTRY
from src.config import MAX_MOVES_PER_GAME


def evaluate_v8_model(model, device, num_games=200, verbose=False,
                      bot_types=None, context_length=16):
    """
    Evaluate V8 model against heuristic bots.
    Uses greedy policy (argmax) with a rolling grid context window.
    """
    model.eval()
    ...
    for game_idx in range(num_games):
        ...
        history = TurnHistoryV8(context_length)
        ...
        while not state.is_terminal and move_count < MAX_MOVES_PER_GAME:
            ...
            if current_player == model_player:
                if len(legal_moves) == 1:
                    ...
                    # Encode state as grid
                    grid = ludo_cpp.encode_state(state)  # (17, 15, 15)
                    history.add_turn(grid, action=last_action, cnn_feature=None)

                    # Cache CNN feature for this new turn only
                    with torch.no_grad():
                        grid_t = torch.from_numpy(grid).unsqueeze(0).to(device, dtype=torch.float32)
                        cnn_feat = model.compute_single_cnn_features(grid_t)  # (1, 128)
                        history._cnn_features[-1] = cnn_feat.cpu().numpy()[0]

                    # Get cached sequence (no re-running CNN on old turns)
    ...
    if verbose:
        print(f"\n{'='*50}")
        print(f"  V8 Evaluation Results ({total} games)")
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Evaluate V8 CNN+Transformer Model')
    parser.add_argument('--checkpoint', type=str, default=None, help='Path to V8 checkpoint')
    parser.add_argument('--v6-weights', type=str, default=None, help='Path to V6 CNN weights (for testing)')
    ...
    elif args.v6_weights:
        model.load_v6_weights(args.v6_weights)

    model.freeze_cnn()
    model.to(device)

    results = evaluate_v8_model(model, device, num_games=args.games, verbose=True)
