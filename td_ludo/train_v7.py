"""
TD-Ludo V7 Training Entry Point — Sequence Transformer Edition

Training loop with:
- AlphaLudoV7 transformer model (1D sequence input, context window K=16)
- PPO (Proximal Policy Optimization) with dense v1 rewards
- Batched execution using VectorGameState + VectorV7GamePlayer
...
# V7 uses its own checkpoint directory
os.environ.setdefault('TD_LUDO_RUN_NAME', 'ac_v7_transformer')

# Add project root
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))

from src.model_v7 import AlphaLudoV7
from src.trainer_v7 import V7Trainer
from src.game_player_v7 import VectorV7GamePlayer
...
def main():
    ...
    parser = argparse.ArgumentParser(description='TD-Ludo V7 Transformer Training')
    ...
    # Initialize model — AlphaLudoV7 Sequence Transformer
    model_factory = lambda: AlphaLudoV7(
        context_length=16, embed_dim=128, num_heads=4, num_layers=4
    )
    model = model_factory()
    print(f"[Train] Architecture: AlphaLudoV7 Transformer (128dim, 4h, 4L, {model.count_parameters():,} params)")
    model.to(device)

    # Initialize trainer
    trainer = V7Trainer(model, device, learning_rate=LEARNING_RATE)
    ...
    # Eval-only mode
    if args.eval_only:
        from evaluate_v7 import evaluate_v7_model
        results = evaluate_v7_model(model, device, num_games=500, verbose=True)
    ...
    # Initialize game player (Vectorized V7 with context window)
    player = VectorV7GamePlayer(
    ...
    print(f"\n{'='*60}")
    print(f"  TD-Ludo V7 Transformer Training — {MODE} Mode")
    ...
    try:
        while not STOP_REQUESTED:
            ...
            if games_since_eval >= EVAL_INTERVAL:
                ...
                eval_results = evaluate_v7_model(model, device, num_games=EVAL_GAMES, verbose=False)
    ...
    print(f"\n{'='*60}")
    print(f"  V7 Training Complete ({MODE})")
