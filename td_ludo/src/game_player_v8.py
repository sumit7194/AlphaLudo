"""
TD-Ludo V8 Game Player — V6 CNN + Temporal Transformer

Key differences from V7 VectorV7GamePlayer:
1. Uses ludo_cpp.encode_state() grid encoding (17, 15, 15) instead of 1D vectors
2. TurnHistoryV8 stores grid tensors instead of (token_positions, continuous)
3. Passes (grids, prev_actions, seq_mask) to model instead of V7's 4 tensors
4. Trajectory stores grid data for the V8 trainer

The game environment, bots, reward shaping, ghost system, and
game composition logic are reused directly from V6/V7.
"""
...
class TurnHistoryV8:
    ...
    Stores the last K turns of (grid, prev_action, cached_cnn_feature).

    CNN features are cached per-turn so only the NEW turn needs CNN processing
    each step (instead of re-processing all K turns).
    """

    def __init__(self, context_length=16, embed_dim=128):
        self.K = context_length
        self.embed_dim = embed_dim
        self._grids = []            # list of np.array(17, 15, 15) float32
        self._actions = []           # list of int (0-4)
        self._cnn_features = []      # list of np.array(embed_dim,) float32 — cached CNN output
    ...
    def add_turn(self, grid, action=4, cnn_feature=None):
        """
        Record a turn with optional cached CNN feature.

        Args:
            grid: np.array(17, 15, 15) float32 — from ludo_cpp.encode_state()
            action: int — the action taken on the PREVIOUS turn (4=none/pass)
            cnn_feature: np.array(embed_dim,) float32 — cached CNN backbone output
        """
    ...
    def get_sequence(self):
        """
        Get the padded K-length sequence for model input (raw grids).

        Returns:
            grids: np.array(K, 17, 15, 15) float32
            prev_actions: np.array(K,) int64
            seq_mask: np.array(K,) bool — True for padded positions
        """
        n_valid = len(self._grids)
        n_pad = self.K - n_valid

        grids = np.zeros((self.K, 17, 15, 15), dtype=np.float32)
        acts = np.full(self.K, 4, dtype=np.int64)  # 4 = pass/none
        mask = np.ones(self.K, dtype=bool)  # True = padding

        if n_valid > 0:
            grids[n_pad:] = np.stack(self._grids)
            acts[n_pad:] = np.array(self._actions, dtype=np.int64)
            mask[n_pad:] = False

        return grids, acts, mask

    def get_cached_sequence(self):
        """
        Get the padded K-length sequence with cached CNN features.

        Returns:
            cached_cnn: np.array(K, embed_dim) float32 — cached CNN features (zeros for padding)
            prev_actions: np.array(K,) int64
            seq_mask: np.array(K,) bool — True for padded positions
        """
        n_valid = len(self._grids)
...
class VectorV8GamePlayer:
    """
    V8 game player with CNN + temporal transformer context window.

    Same structure as VectorV7GamePlayer but stores grid states
    and passes (grids, prev_actions, seq_mask) to model.
    """
    ...
    def __init__(self, trainer, batch_size, device, context_length=16,
        ...
        for i in range(batch_size):
            for p in range(4):
                self.turn_histories[(i, p)] = TurnHistoryV8(context_length)
    ...
    # --- Ghost management (same as V6/V7) ---
    ...
    def play_step(self, train=True):
        """Advance all games by one step. Returns list of finished game results."""
        actions = []
        current_players = []
        decision_groups = {}
        ...
        if decision_groups:
            ...
            for (controller, controller_id), indices in decision_groups.items():
                batch_sequences = []
                batch_cached_sequences = []
                batch_new_grids = []
                batch_new_grid_indices = []
                batch_legal_masks = []
                batch_legal_moves = []

                for j_local, idx in enumerate(indices):
                    game = self.env.get_game(idx)
                    cp = current_players[idx]
                    lmoves = ludo_cpp.get_legal_moves(game)
                    batch_legal_moves.append(lmoves)

                    # Encode current state as grid
                    grid = ludo_cpp.encode_state(game)  # (17, 15, 15)

                    # Add current turn to history (CNN feature computed below)
                    ...
                    # Track this new grid for batched CNN processing
                    batch_new_grids.append(grid)
                    batch_new_grid_indices.append(j_local)
                ...
                use_cache = getattr(model, 'cnn_frozen', True)

                if use_cache:
                    # Batch-compute CNN features for NEW turns only (not all K turns!)
                    with torch.no_grad():
                        new_grids_t = torch.from_numpy(
                            np.stack(batch_new_grids)
                        ).to(self.device, dtype=torch.float32)  # (B, 17, 15, 15)
                        new_cnn_features = model.compute_single_cnn_features(new_grids_t)  # (B, 128)
                        new_cnn_np = new_cnn_features.cpu().numpy()

                    # Store cached CNN features in histories and build cached sequences
                    for j_local, idx in enumerate(indices):
                        cp = current_players[idx]
                        history = self.turn_histories[(idx, cp)]
                        history._cnn_features[-1] = new_cnn_np[j_local]

                        cached_cnn, seq_acts, seq_mask = history.get_cached_sequence()
                        batch_cached_sequences.append((cached_cnn, seq_acts, seq_mask))

                        seq_grids, _, _ = history.get_sequence()
                        batch_sequences.append((seq_grids, seq_acts, seq_mask))

                else:
                    # CNN unfrozen — can't use cached features (weights change each update)
                    for j_local, idx in enumerate(indices):
                        cp = current_players[idx]
                        history = self.turn_histories[(idx, cp)]
                        seq_grids, seq_acts, seq_mask = history.get_sequence()
                        batch_sequences.append((seq_grids, seq_acts, seq_mask))

                # Stack into batched tensors
                B = len(indices)
                masks_t = torch.from_numpy(
                    np.stack(batch_legal_masks)
                ).to(self.device, dtype=torch.float32)  # (B, 4)

                if use_cache:
                    all_cached = torch.from_numpy(
                        np.stack([s[0] for s in batch_cached_sequences])
                    ).to(self.device, dtype=torch.float32)  # (B, K, 128)
                    all_acts = torch.from_numpy(
                        np.stack([s[1] for s in batch_cached_sequences])
                    ).to(self.device)  # (B, K) int64
                    all_mask = torch.from_numpy(
                        np.stack([s[2] for s in batch_cached_sequences])
                    ).to(self.device)  # (B, K) bool
                    ...
                    all_grids = torch.from_numpy(
                        np.stack([s[0] for s in batch_sequences])
                    ).to(self.device, dtype=torch.float32)  # (B, K, 17, 15, 15)
                    all_acts = torch.from_numpy(
                        np.stack([s[1] for s in batch_sequences])
                    ).to(self.device)
                    all_mask = torch.from_numpy(
                        np.stack([s[2] for s in batch_sequences])
                    ).to(self.device)

                    with torch.no_grad():
                        policy_logits = model.forward_policy_only(
                            all_grids, all_acts, all_mask, masks_t
                        )
                ...
                for j, idx in enumerate(indices):
                    ...
                    self.trajectories[idx][cp].append({
                        'grids': batch_sequences[j][0].copy(),       # (K, 17, 15, 15)
                        'prev_actions': batch_sequences[j][1].copy(), # (K,)
                        'seq_mask': batch_sequences[j][2].copy(),    # (K,)
                        'action': action,
    ...
    def _random_composition(self):
        """Generate random game composition (same logic as V6/V7)."""
