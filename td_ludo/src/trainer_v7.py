"""
TD-Ludo V7 Trainer — PPO for Sequence Transformer

Same PPO algorithm as V6 trainer, but handles V7's sequence-based inputs:
- Trajectories contain (token_positions, continuous, actions_seq, seq_mask)
  per step instead of a single (17, 15, 15) state tensor.
- model.forward() takes these 4 tensors + legal_mask.

All other logic (PPO clipping, advantage normalization, gamma-discounted returns,
ghost saving, checkpoint management) is identical to V6.
"""
...
class V7Trainer:
    """
    PPO trainer for AlphaLudoV7 (Sequence Transformer).

    Same interface as ActorCriticTrainer but handles V7's sequence inputs.
    """
    ...
    def train_on_game(self, trajectories, winner, model_player):
        ...
        V7 trajectories contain sequence data per step:
        - token_positions: (K, 8)
        - continuous: (K, 9)
        - actions_seq: (K,)
        - seq_mask: (K,)
        ...
        for i, step in enumerate(trajectory):
            self._ppo_buffer.append({
                'token_positions': step['token_positions'],  # (K, 8)
                'continuous': step['continuous'],            # (K, 9)
                'actions_seq': step['actions_seq'],          # (K,)
    ...
    def _ppo_update(self):
        ...
        # Stack all sequence data
        all_tok = torch.from_numpy(
            np.stack([s['token_positions'] for s in self._ppo_buffer])
        ).to(self.device)  # (N, K, 8) int64

        all_cont = torch.from_numpy(
            np.stack([s['continuous'] for s in self._ppo_buffer])
        ).to(self.device, dtype=torch.float32)  # (N, K, 9)

        all_acts_seq = torch.from_numpy(
            np.stack([s['actions_seq'] for s in self._ppo_buffer])
        ...
        ).to(self.device, dtype=torch.float32)  # (N, 4)
        ...
        with torch.no_grad():
            _, all_values = self.model(all_tok, all_cont, all_acts_seq, all_seq_mask, all_masks)
        ...
        for epoch in range(self.ppo_epochs):
            ...
            for start in range(0, n_steps, self.ppo_minibatch_size):
                ...
                mb_tok = all_tok[mb_idx]
                mb_cont = all_cont[mb_idx]
                mb_acts_seq = all_acts_seq[mb_idx]
                ...
                policy, value = self.model(
                    mb_tok, mb_cont, mb_acts_seq, mb_seq_mask, mb_masks
                )
    ...
    def flush_buffer(self):
        if self._ppo_buffer:
            print(f"[V7Trainer] Flushing PPO buffer ({self._ppo_games_buffered} games, {len(self._ppo_buffer)} steps)")
            self._ppo_update()

    # --- Stats accessors (same as V6) ---
    ...
    def log_metrics(self, win_rate, games_played, eval_win_rate=None):
        ...
        try:
            ...
            print(f"[V7Trainer] Failed to save metrics: {e}")

    # --- Checkpoint management (same as V6) ---
            ...
            if isinstance(checkpoint, dict) and 'model_state_dict' in checkpoint:
                if 'optimizer_state_dict' in checkpoint:
                    try:
                        ...
                        print(f"[V7Trainer] Warning: Could not load optimizer state: {e}")
                ...
                print(f"[V7Trainer] Loaded checkpoint: {self.total_games} games, {self.total_updates} updates")
            else:
                self.total_updates = 0
                self.total_games = 0
                self.best_win_rate = 0.0
                self.last_ghost_game = 0
                self.metrics_history = []
                print(f"[V7Trainer] Loaded raw model weights (fresh start)")
            return True
        except Exception as e:
            print(f"[V7Trainer] Failed to load checkpoint: {e}")
            return False
